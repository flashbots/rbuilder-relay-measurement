...
def fetch_data(builder_pubkey, block_number=None):
    ...
    if os.path.exists(filepath):
        ...
        params = {"builder_pubkey": builder_pubkey, "block_number": block_number}
...
def compare_data(file_path, fetched_data):
    ...
    for i, item in enumerate(file_data):
        ...
        if timestamp_file and timestamp_fetched:
            timestamp_file_obj = datetime.datetime.strptime(
                timestamp_file, "%Y-%m-%dT%H:%M:%S.%fZ"
            )
            timestamp_file_ms = (
                timestamp_file_obj.replace(tzinfo=datetime.timezone.utc).timestamp()
                * 1000
            )
        ...
        if num_transactions_file and num_transactions_fetched:
            if int(num_transactions_file) != int(num_transactions_fetched):
                print(
                    f"Number of transactions for item {i+1} is different , {num_transactions_file} and {num_transactions_fetched}"
                )

        if item.get("spans")[1].get("optimistic") != fetched_item.get(
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print(
            "Usage: python script.py <builder_pubkey> <block_numbery> <rbuilder_logs>"
        )
        sys.exit(1)
