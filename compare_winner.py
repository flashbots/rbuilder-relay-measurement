...
def fetch_and_compare(slot, builder_data_list):
    ...
    if data:
        ...
        for i, builder_data in enumerate(builder_data_list):
            ...
            if builder_data['builder_pubkey'] == winner_data[0]['builder_pubkey']:
                print("Your builder pubkey won this slot!")
                break
            print(f"Value difference: {int(winner_data[0]['value']) - int(builder_data['value'])}")
            print(f"Gas used difference: {int(winner_data[0]['gas_used']) - int(builder_data['gas_used'])}")
            print(f"Num_tx difference: {int(winner_data[0]['num_tx']) - int(builder_data['num_tx'])}")
            print(f"Timestamp difference: {int(winner_data[0]['timestamp_ms']) - int(builder_data['timestamp_ms'])}")
...
def main():
    ...
    parser.add_argument('--limit', type=int, default=500, help='Limit for the number of records to fetch')
