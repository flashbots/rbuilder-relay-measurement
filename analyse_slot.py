...
def compare_data(builder_pubkey, slot, base_url=None):
    ...
    for i, item in enumerate(filtered_data):
        print(f"Bid {i+1}:")
        print(
            f"    Value difference: {int(winner_full_data['value']) - int(item['value'])}"
        )
        print(
            f"    Gas used difference: {int(winner_full_data['gas_used']) - int(item['gas_used'])}"
        )
        print(
            f"    Num_tx difference: {int(winner_full_data['num_tx']) - int(item['num_tx'])}"
        )
        print(
            f"    Timestamp_ms difference: {int(winner_full_data['timestamp_ms']) - int(item['timestamp_ms'])}"
        )


def main():
    parser = argparse.ArgumentParser(
        description="Compare builder data with winner data."
    )
